...
fn read_user_from_file<P: AsRef<Path>>(path: P) -> Result<UserReq, Box<dyn Error>> {
    // Open the file in read-only mode with buffer.
    let file = StdFile::open(path)?;
    let reader = BufReader::new(file);

    // Read the JSON contents of the file as an instance of `User`.
    let u: UserReq = serde_json::from_reader(reader)?;
...
async fn main() {
    ...
    //read json
    let u: UserReq  = read_user_from_file("test.json").unwrap();

    // read command line args
    // store argument state to pass into the function
    let test;
    ...
    match args.peek().map(|x| x.as_ref()) {
        ...
        Some("--option1") => {
            ...
            test = [u.useragent.as_bytes(),u.host.as_bytes(),u.accept.as_bytes(),u.uri.as_bytes(),&[0u8]];

        }
        Some("--option2") => {
            args.next(); // Skip the flag
            println!("handle option 2");
            test = [u.body.as_bytes(),&[0u8],&[0u8],&[0u8],&[0u8]];
        }
        Some("--option3") => {
            args.next(); // Skip the flag
            println!("handle option 3");
            test = [u.body.as_bytes(),u.useragent.as_bytes(),u.host.as_bytes(),u.accept.as_bytes(),u.uri.as_bytes()];
        }
        _ => {
            println!("handle no option");
            test = [&[0u8],&[0u8],&[0u8],&[0u8],&[0u8]];
        }
    }
    ...
    let client_socket = tokio::net::TcpStream::connect((u.host.to_owned(), 443))
        .await
        .unwrap();
    ...
    let request = Request::builder()
        .uri(u.uri.to_owned())
        .header("Host", u.host.to_owned())
        .header("Accept", u.accept.to_owned())
        // Using "identity" instructs the Server not to use compression for its HTTP response.
        // TLSNotary tooling does not support compression.
        .header("Accept-Encoding", "identity")
        .header("Connection", "close")
        .header("User-Agent", u.useragent.to_owned())
        .body(Body::from(u.body.to_owned()))
        .unwrap();
